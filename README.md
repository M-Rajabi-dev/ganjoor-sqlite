...
- `ganjoor.sqlite`: Raw, uncompressed SQLite database (ready for direct use).
- `ganjoor.sqlite.zip`: Standard ZIP archive.
- `ganjoor.sqlite.7z`: Compressed with 7-Zip LZMA2 (maximum compression, smallest file size).

To decompress:
```bash
unzip ganjoor.sqlite.zip
```
```bash
7z x ganjoor.sqlite.7z
```
...
python build_db.py --data-dir ganjoor-data-main --output ganjoor.sqlite
```
